<template name="liveFollow">
  <block v-for="(item, index) in authorList" :key="index">
    <view class="fllow_list_item">
      ...
      <view class="right">
      <text @tap="collect" :data-index="index" :data-authorId="item.author_id" :class="'fllow ' + (item.is_follow == 1 ? 'has_fllow' : '')">{{item.is_follow == 1 ? $L('已关注') : $L('关注')}}</text>
      <text v-if="false" class="fans_num">{{item.fans_num}}{{$L('粉丝')}}</text>
      </view>
    </view>
    <view v-if="authorList.length - index > 1" class="line_marginl_20"></view>
  </block>
</template>