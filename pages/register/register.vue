...
<template>
	<view class="forget_container">
		...
		<view class="wrapper">
			...
			<!-- <view class="login-title1" @click="navTo('/pages/register/registerel',1)">
				{{$L('邮箱注册')}}
			</view> -->
			...
			<view class="input-content">
				<view class="input-item "><!-- input-item1 -->
					<!-- <picker @change="changeCountryCode" :value="currentCodeIndex" :range="countryCodeArray" range-key="country_code">
						<view class="uni-input" style="text-align: center;">{{countryCodeArray[currentCodeIndex]?countryCodeArray[currentCodeIndex].country_code:'请选择'}}</view>
					</picker> -->
					<input type="number" :value="mobile" :placeholder="$L('请输入手机号')" maxlength="11" data-key="mobile" @input="inputChange"
					 @focus="setFocus" /><!-- style="margin-left: 10px;" -->
			...
			<button class="confirm-btn" @click="toRegist" :style="{opacity: (!(check_agreement&&mobile&&smsCode&&imgCode)||logining)?0.5:1}">{{$L('注册')}}</button>
			<view class="login-register">
				<text class="mobile-login">{{$L('已有账号～')}}</text>
				<text class="register" @click="navTo('/pages/login/login',1)">{{$L('去登陆')}}></text>
			</view>
		</view>
		<view class="other-login" v-if="showOtherFlag&&is_show!=1">
			<view class="title">
				<text>{{$L('其他登录')}}</text>
			</view>
		...
		methods: {
			...
			getshow(){
				uni.request({
					...
					success: res => {
						this.is_show = res.data.shenhe;
						// console.log(res.data.shenhe)
					}
				});
			},
			// 区号
			...
			toRegist() {
				...
				this.logining = true;
				uni.request({
					url: getApp().globalData.ser_url + '/index.php?app=login_mobile&mod=mobileregister',
					method: 'POST',
					data: {
						'mobile': this.mobile,
