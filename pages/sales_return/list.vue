...
<view class="list" v-if="refundList.length">
  <view v-for="(item, index) in refundList" :key="index" class="item">
    ...
    <view class="btns">
      ...
      <navigator :url="'/pages/refund/refund?id=' + item.refund_id" hover-class="none">{{$L('退款详情')}}</navigator>
    </view>
  </view>
</view>
...
export default {
  ...
  methods: {
    initData() {
      ...
      let url = type == 1 ? getApp().globalData.ser_url + `/index.php?app=userorder&mod=refundList&key=${key}&pn=${this.pn}&page=10` : getApp().globalData.ser_url + `/index.php?app=userorder&mod=returnList&key=${key}&pn=${this.pn}&page=10`;
      request({
        url
      }).then(res => {
        if (res.code == 200) {
