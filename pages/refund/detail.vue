  ...
  <view class="order_list">
    ...
    <view class="refund_info">
      <view class="refund_item">
        <view class="title">{{$L('退款原因')}}</view>
        <input type="text" :placeholder="$L('请填写退款原因')" :value="tyuan" :disabled="type==1" @input="yInput"></input>
      </view>
      ...
      <view class="refund_item" v-if="type==1">
        ...
        <input type="text" :placeholder="$L('请填写原因')" @input="y2Input"></input>
      </view>
  ...
  methods: {
    ...
    sumbit() {
      ...
      if (type == 1) {
        ...
        uni.request({
          url: getApp().globalData.ser_url + '/index.php?app=userorder&mod=add_refund_all',
          method: 'POST',
          header: {
